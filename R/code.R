...
cwm2 <- function(formulaY, data, Y, Xnorm, Xmult, Xpois, Xbin,colXm,colXn,colXb,colXp,Xbtrials, n, m, Xmod, k, modelXnorm, familyY,familyYname,mY, 
                    method, initialization, start.z, iter.max, threshold, loglikplot, seed,maxR) 
  ...
  # m vector with number of levels of factors
{
  ...
  if(!is.null(data)) if(familyYname=="t"){ 
  ...
  for (r in 1:maxR){
    ...
    while(check<1){
      ...
      norm  <- .PX_norm(colXn=colXn,Xnorm=Xnorm,modelXnorm=modelXnorm,z=z,k=k,n=n)
      ...
      if(iteration>2 & k > 1){
        if (!is.na(loglik[iteration-1]-loglik[iteration-2])){
    ...
    if(!is.null(data)){
      ...
      if(familyYname=="t")  df_model <- df_model + 2*k
...
## GLM parameters 
    ...
    if(!is.null(data)){
      ...
      for (i in seq_len(k)) {
        ...
        if (familyYname=="gaussian" |familyYname=="t") parameters <- c(parameters, list(sigma = sqrt(l$VarY[1,i])))
        if (familyYname=="t") parameters <- c(parameters, list(t_df = t_df[i]))
